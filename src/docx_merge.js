...
import './docx_merge.css'; // Import the CSS file here
...
const DOCXMerger = () => {
    ...
    const handleMerge = async () => {
        try {
            ...
            const response = await axios.post('https://flaskend.onrender.com/api/mergeDocx', formData, {
    ...
    const handleDownload = () => {
        // Create a blob URL to download the merged file
    ...
    return (
        ...
        <div
            ...
            onDrop={handleDrop}
        >
            <h1 className="title">Word Merge</h1>
            <div className={`drop-area ${dragging ? 'drag-over' : ''}`}>
                {file1 && file2 ? (
                    <p>Two files selected</p>
                ) : (
                    <p>Drag and drop Word files here</p>
                )}
            </div>
            <input type="file" onChange={handleFile1Change} />
            <input type="file" onChange={handleFile2Change} />
            <button
                className={`converter-button ${scrolling ? 'scrolling' : ''}`}
                onClick={handleMerge}
            >
                Merge Files
            </button>
